...
export function openAiChat({ getCode, onInsert }) {
    if (_activeOverlay) {
        _activeOverlay.querySelector('.eai-input-ta')?.focus();
        return;
    }
    ...
    overlay.innerHTML = `
        <div class="eai-panel">
            <div class="eai-header">
                <span class="eai-title">AI Indicator Assistant</span>
                <button class="icon-btn eai-close">&times;</button>
            </div>

            <div class="eai-controls">
                <select class="eai-model-select">
    ...
    const panel = overlay.querySelector('.eai-panel');
    ...
    fetch(window.ARI.api + '?action=init')
        .then(r => r.json())
        .then(data => {
            ...
            if (data.models?.length) {
                ...
                modelSelect.innerHTML = `<option disabled>No free models available</option>`;
            }
        })
        .catch(() => {
    ...
    instrTa.oninput = () =>
        localStorage.setItem(CUSTOM_KEY, instrTa.value);
